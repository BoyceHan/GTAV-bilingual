#- (english) Usage Guideline  -#
	This is an example guideline showing how to merge "american.rpf" (US English) and "chinesesimp.rpf" (Simplified Chinese)
	...
	4. Turn to ./update/update.rpf/x64/data/lang
	
	5. In this directory you will find language packages such as "american.rpf" and "chinesesimp.rpf"
	
	6. Put this .py script in a directory (let us call it "./")
	
	7. Create a directory under "./" called "american", open "american.rpf" and extract all .gxt2 files to "./american/"
	
	8. Create a directory under "./" called "chinesesimp", open "chinesesimp.rpf" and extract all .gxt2 files to "./chinesesimp/"
	...
	13. Return to OpenIV, delete all .gxt2 files in "chinesesimp.rpf" (we recommand you backup before deleteing, but they're already backed-up in "./chinesesimp", right? (laugh) )
	    and add all .gxt2 files in "./merged/" into "chinesesimp.rpf"
	
		WARNING: you may want to delete all .gxt2 files in "american.rpf" and add all .gxt2 files in "./merged/" into it.
		(so you have Chinese version and Chinese-English-bilingual version; anyway you do not play English-Solo version, right?)
		DO NOT DO SO.
		KEEP "american.rpf" NOT MODIFIED ASSURES THAT YOU CAN ALWAYS RUN ENGLISH LANGUAGE VERSION OF GTAV CORRECTLY.
...
#- (chinese) 使用教程  -#
	此为示例教程，说明如何把 "american.rpf" （美式英语）和 "chinesesimp.rpf" （简体中文合并）
	...
	4. 进入目录 ./update/update.rpf/x64/data/lang
	
	5. 在此目录中您将找到若干语言包，其名称类似于 "american.rpf" 和 "chinesesimp.rpf"
	
	6. 把此.py脚本放在一个独立的目录中（下文把这个目录位置称为"./"）
	
	7. 在 "./" 下创建一个名为 "american" 的新目录, 打开 "american.rpf" 并提取所有 .gxt2 文件至 "./american/" 目录
	
	8. 在 "./" 下创建一个名为 "chinesesimp" 的新目录, 打开 "chinesesimp.rpf" 并提取所有 .gxt2 文件至 "./chinesesimp/" 目录
	...
	13. 回到OpenIV, 删除 "chinesesimp.rpf" 中的所有 .gxt2文件 （我们建议您删除前先备份，但刚刚已经备份到"./chinesesimp/"里了不是吗？（笑））
		并将 "./merged/" 目录中所有 .gxt2 文件 导入 "chinesesimp.rpf"
	
		警告: 您可能想删除"./american/.rpf"文件中所有.gxt2文件并把"./merged/"中的所有.gxt2文件导入进去
		（这样就有中文、中英两个版本玩了！反正我也不玩纯英文版。）
		不要这样做，绝对不要。
		保持 "american.rpf" 不变，这能保证英语版本永远是正确无误的。英语版本是GTAV最稳定的版本，运行英语版不依赖其他语言包。反之不然，不然！
